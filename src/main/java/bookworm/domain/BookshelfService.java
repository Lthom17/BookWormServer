...
public class BookshelfService {
    ...
    public boolean deleteBookshelf(UUID bookshelfId) {
        ...
        List<Bookshelf> bookshelvesToDelete = BOOKSHELF_REPOSITORY.findByBookshelfParentId(bookshelfId);

        if(bookshelvesToDelete.size() > 0) {
            for(Bookshelf b : bookshelvesToDelete) {
                if(BOOK_REPOSITORY.deleteByBookshelfId(b.getBookshelfId())) {
                    bookDeleted = true;
                }
                else
                    bookDeleted = false;

                if(BOOKSHELF_REPOSITORY.delete(b.getBookshelfId())) {
                    bookshelfDeleted = true;
                }
                else
                    bookshelfDeleted = false;
            }
        }
